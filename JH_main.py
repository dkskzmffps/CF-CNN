...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=num_workers, pin_memory=False)
testloader  = torch.utils.data.DataLoader(testset, batch_size=args.test_batch_size, shuffle=False, num_workers=num_workers, pin_memory=False)
...
group_title = '_h3_G-01-100-487_S-01-100-05_L-01-01-(10e-5)'
# group_title = '_h3_G-01-05-25_S-01-05-05_L-01-01-(10e-5)'
...
if (args.save_class_score):
    if args.dataset == 'imagenet':
        trainset = datasets.ImageFolder(train_dir, transform=transform_test)
        testset = datasets.ImageFolder(test_dir, transform=transform_test)

    elif args.dataset.startswith('cifar'):
        trainset = JH_utile.MyImageDataSet(train_data, train_labels, transform=transform_test)
        testset = JH_utile.MyImageDataSet(test_data, test_labels, transform=transform_test)

    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=False, num_workers=num_workers,
                                              pin_memory=False)
