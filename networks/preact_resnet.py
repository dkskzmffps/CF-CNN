...
class PreActResNet(nn.Module):
    def __init__(self, depth_list, num_classes_list, dataset, bottleneck=False):
        ...
        if self.dataset.startswith('cifar'):
            ...
            self.layer1 = self._make_layer(block_type[depth], 64,  layers[depth][0])
            self.layer2 = self._make_layer(block_type[depth], 128, layers[depth][1], stride=2)
            self.layer3 = self._make_layer(block_type[depth], 256, layers[depth][2], stride=2)
            self.layer4 = self._make_layer(block_type[depth], 512, layers[depth][3], stride=2)
            self.bn2    = nn.BatchNorm2d(512 * block_type[depth].expansion)
            self.fc     = nn.Linear (512 * block_type[depth].expansion, num_classes)
