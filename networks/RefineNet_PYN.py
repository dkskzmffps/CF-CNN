...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if residual_channel != shortcut_channel:
            padding = torch.autograd.Variable(torch.cuda.FloatTensor(batch_size,
...
class RefineNet_PyramidNet(nn.Module):
    def __init__(self, depth_list, num_classes_list, dataset, alpha, bottleneck=False):
        super(RefineNet_PyramidNet, self).__init__()
        ...
        if self.dataset.startswith('cifar'):
            ...
            if bottleneck == False:
                ...
                n_sub1 = depth_list[1] // 2  # total number of residual units of sub net, one block has 2 layer
                ...
                n_sub1 = depth_list[1] // 3  # total number of residual units of sub net, one block has 3 layer
            ...
            self.layer_merging = self._make_layer_merging(block_type, 2)
            assert self.in_planes == out_featuremap_dim, 'please check Addrate and layer number'
            ...
            for i in range(len(depth_list)):
                assert layers[depth_list[i]], 'invalid depth for Pre-ResNet (depth should be one of 18, 34, 50, 101, 152, and 200)'
            assert block_type[depth_list[0]] == block_type[depth_list[1]], 'please check block type'

            n_main = layers[depth_list[0]]
            n_sub1 = layers[depth_list[1]]


            self.in_planes = 64
            self.addrate = alpha / (1 * sum(layers[depth_list[0]]) * 1.0)
            self.addrate_sub1 = 0
            self.addrate_sub2 = 0

            out_planes_block3 = self.in_planes + (self.addrate * sum(layers[depth_list[0]]))
            out_featuremap_dim = int(round(out_planes_block3)) * block_type[depth_list[0]].outchannel_ratio
            self.in_planes_sub1      = self.in_planes
            self.featuremap_dim      = self.in_planes
            self.featuremap_dim_sub1 = self.in_planes

            self.conv0  = conv7x7(3, self.in_planes, stride=2)
            self.bn0    = nn.BatchNorm2d(self.in_planes)

            self.layer1 = self._make_layer(block_type[depth_list[0]], n_main[0])
            self.featuremap_dim_sub1 = self.featuremap_dim
            self.addrate_sub1 = (out_planes_block3 - self.featuremap_dim_sub1) / (1 * sum(n_sub1[1:4]) * 1.0)
            self.in_planes_sub1 = self.in_planes

            self.layer1_sub1 = self._make_layer_sub(block_type[depth_list[0]], n_sub1[1], stride=2)
            self.layer1_sub2 = self._make_layer_sub(block_type[depth_list[0]], n_sub1[2], stride=2)
            self.layer1_sub3 = self._make_layer_sub(block_type[depth_list[0]], n_sub1[3], stride=2)
            self.bn1 = nn.BatchNorm2d(self.in_planes_sub1)
            self.fc1 = nn.Linear(self.in_planes_sub1, num_classes_list[0])
            assert self.in_planes_sub1 == out_featuremap_dim, 'please check addrate and layer number'

            self.layer2 = self._make_layer(block_type[depth_list[0]], n_main[1], stride=2)
            self.featuremap_dim_sub1 = self.featuremap_dim
            self.addrate_sub1 = (out_planes_block3 - self.featuremap_dim_sub1) / (1 * sum(n_sub1[2:4]) * 1.0)
            self.in_planes_sub1 = self.in_planes

            self.layer2_sub1 = self._make_layer_sub(block_type[depth_list[0]], n_sub1[2], stride=2)
            self.layer2_sub2 = self._make_layer_sub(block_type[depth_list[0]], n_sub1[3], stride=2)
            self.bn2 = nn.BatchNorm2d(self.in_planes_sub1)
            self.fc2 = nn.Linear(self.in_planes_sub1, num_classes_list[1])
            assert self.in_planes_sub1 == out_featuremap_dim, 'please check addrate and layer number'

            self.layer3 = self._make_layer(block_type[depth_list[0]], n_main[2], stride=2)
            self.layer4 = self._make_layer(block_type[depth_list[0]], n_main[3], stride=2)
            self.bn3 = nn.BatchNorm2d(self.in_planes)
            self.fc3 = nn.Linear(self.in_planes, num_classes_list[2])
            assert self.in_planes == out_featuremap_dim, 'please check addrate and layer number'

            self.layer_merging = self._make_layer_merging(block_type[depth_list[0]],2)
            assert self.in_planes == out_featuremap_dim
            self.bn_merging = nn.BatchNorm2d(self.in_planes)
            self.fc_merging = nn.Linear(self.in_planes, num_classes_list[2])
    ...
    def forward(self, x):
        if self.dataset.startswith('cifar'):
            ...
            sub1 = self.layer1_sub1(out)
            sub1 = self.layer1_sub2(sub1)
            sub1 = self.layer1_sub3(sub1)
            sub1 = self.bn1(sub1)
            sub1 = F.relu(sub1, inplace=True)
            out_sub1 = F.avg_pool2d(sub1, 7)
            out_sub1 = out_sub1.view(out_sub1.size(0), -1)
            out_sub1 = self.fc1(out_sub1)

            out = self.layer2(out)
            sub2 = self.layer2_sub1(out)
            sub2 = self.layer2_sub2(sub2)
            sub2 = self.bn2(sub2)
            sub2 = F.relu(sub2, inplace=True)
            out_sub2 = F.avg_pool2d(sub2, 7)
            out_sub2 = out_sub2.view(out_sub2.size(0), -1)
            out_sub2 = self.fc2(out_sub2)

            sub3 = self.layer3(out)
            sub3 = self.layer4(sub3)
            sub3 = self.bn3(sub3)
            sub3 = F.relu(sub3, inplace=True)
            out_sub3 = F.avg_pool2d(sub3, 7)
            out_sub3 = out_sub3.view(out_sub3.size(0), -1)
            out_sub3 = self.fc3(out_sub3)
