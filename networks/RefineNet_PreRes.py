...
def feature_resize(size, method = 'bilinear'):
    return nn.Upsample(size=size, mode=method)
...
class RefineNet_PreActResNet(nn.Module):
    def __init__(self, depth_list, num_classes_list, dataset, bottleneck=False):
        super(RefineNet_PreActResNet, self).__init__()
        ...
        if self.dataset.startswith('cifar'):
            ...
            self.bn2 = nn.BatchNorm2d(64 * block_type.expansion)
            ...
            self.bn3 = nn.BatchNorm2d(64 * block_type.expansion)
            ...
            self.bn_merging = nn.BatchNorm2d(64 * block_type.expansion)
